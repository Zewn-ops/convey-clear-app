...
export default async function PartnerMatterDetail({ params }: { params: { id: string } }) {
  ...
  const [{ data: docsData }, { data: actData }] = await Promise.all([
    supabase.from("documents").select("id, document_type, document_status, file_name, uploaded_at, verified").eq("matter_id", params.id),
    supabase.from("matter_activities").select("id, content, activity_type, created_at").eq("matter_id", params.id).order("created_at", { ascending: false }).limit(20),
  ]);
  const docs = (docsData as MatterDocument[] | null) ?? [];
  const activities = (actData as { id: string; content: string; activity_type: string; created_at: string }[] | null) ?? [];
            ...
            <ul className="space-y-2">
              {docs.map((d) => (
                <li key={d.id} className="flex items-center gap-2 text-sm">
                  ...
                  <span className="flex-1 text-gray-700">{d.file_name || d.document_type}</span>
