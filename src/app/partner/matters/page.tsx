...
export default async function PartnerMatters({
  ...
  const { data, count } = await applyMatterFilters(
    supabase
      ...
      .select("id, title, current_phase, current_stage, status, municipality, service_subtype, created_at, updated_at, firm_review_state, clients(full_name, business_name), services(code, name), property_transfers(id, reference)", {
        count: "exact",
      }),
    filters
  );
