...
export function DocSlot({
  ...
  return (
    <div className={cn("rounded-xl border bg-surface shadow-sm transition-all", isComplete ? "border-green-200" : "border-line")}>
      <div className="p-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              {isComplete ? (
                ...
                <div className="h-4 w-4 rounded-full border-2 border-line shrink-0" />
              )}
              <span className="text-sm font-medium text-ink">{meta.label}</span>
            </div>
            {meta.hint && <p className="mt-0.5 ml-6 text-xs text-ink-3">{meta.hint}</p>}
            {templateUrl && (
              <a href={templateUrl} target="_blank" rel="noopener noreferrer" className="mt-1 ml-6 inline-flex items-center gap-1 text-xs text-action hover:underline">
                Download template →
              </a>
            )}
          </div>
          {allowNotAvailable && !state.file && (
            <button
              ...
              className={cn(
                ...
                state.notAvailable ? "border-amber-300 bg-amber-50 text-amber-700" : "border-line text-ink-3 hover:border-line hover:text-ink-2"
              )}
            >
        ...
        {state.notAvailable ? (
          <div className="ml-6 space-y-2">
            ...
            <textarea
              ...
              rows={2}
              className={cn(
                "w-full rounded-lg border px-3 py-2 text-sm text-ink placeholder:text-ink-3 resize-none focus:outline-none focus:ring-2 focus:ring-[#1B2E6B] focus:border-transparent",
                state.reason.trim() ? "border-line" : "border-amber-300"
              )}
            />
          </div>
        ) : (
          <div
            ...
            className={cn(
              ...
              state.file ? "border-green-300 bg-green-50" : "border-line hover:border-line hover:bg-orange-50"
            )}
          >
            ...
            {state.file ? (
              <>
                ...
                <button type="button" onClick={(e) => { e.stopPropagation(); onChange({ file: null }); if (fileRef.current) fileRef.current.value = ""; }}
                  className="absolute top-2 right-2 rounded-full bg-surface p-0.5 shadow text-ink-3 hover:text-ink-2">
              ...
              <>
                ...
                <div>
                  <p className="text-xs font-medium text-ink-2">Click or drag file here</p>
                  <p className="text-xs text-ink-3">PDF, JPG, PNG — max 10 MB</p>
                </div>
              </>
            )}
          </div>
...
const inputCls =
  "w-full rounded-lg border border-line bg-surface px-3 py-2 text-sm text-ink placeholder:text-ink-3 focus:outline-none focus:ring-2 focus:ring-[#1B2E6B] focus:border-transparent";
      ...
      <main className="min-h-screen bg-raised flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center p-6">
          <div className="w-full max-w-md text-center">
            ...
            <h1 className="text-2xl font-semibold text-ink mb-2">Onboarding Submitted</h1>
        ...
        <div className="flex items-center justify-between">
          {STAGES.map((label, i) => (
            <div key={label} className="flex items-center gap-2">
              ...
              <span className={cn("text-xs hidden sm:block", i === stage ? "text-ink font-medium" : "text-ink-3")}>{label}</span>
            </div>
          ))}
        </div>
          ...
          <div className="space-y-3">
            <div className="bg-surface rounded-xl shadow-sm dark:ring-1 dark:ring-line p-4">
              ...
              <div className="h-1.5 bg-raised rounded-full overflow-hidden">
                <div className="h-full bg-action-fill rounded-full transition-all duration-500"
            ...
            {requiredDocs.length === 0 && <p className="text-sm text-ink-3 text-center py-6">No documents required at this stage.</p>}
          </div>
        )}
    ...
    <label className="flex items-start gap-3 cursor-pointer bg-surface rounded-xl shadow-sm dark:ring-1 dark:ring-line p-4">
      ...
      <div>
        <p className="text-sm font-medium text-ink">
          {title} {required && <span className="text-action">*</span>} {optional && <span className="text-xs font-normal text-ink-3">(optional)</span>}
        </p>
        <p className="text-xs text-ink-3 mt-0.5 leading-relaxed">{body}</p>
      </div>
    </label>
  );
}
