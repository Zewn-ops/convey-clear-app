    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-[40px] font-semibold leading-[1.06] tracking-[-0.032em] text-ink">{firm.name}</h1>
            {firm.abbreviation && (
              <span className="font-mono text-xs text-ink-3 bg-raised rounded px-1.5 py-0.5">{firm.abbreviation}</span>
            )}
        ...
        {canWrite && (
          <Link
            ...
            className="inline-flex items-center gap-2 rounded-lg border border-line px-4 py-2 text-sm font-medium text-ink-2 hover:bg-raised"
          >
      ...
      <Card className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        {firm.notes && (
          <div className="sm:col-span-2">
            ...
            <p className="text-sm text-ink-2 mt-1 whitespace-pre-wrap">{firm.notes}</p>
          </div>
        )}
      </Card>
      ...
      <Card padding="none">
        <div className="flex items-center justify-between px-5 py-3 border-b border-line">
          ...
          {canWrite && (
            <Link href="/admin/users" className="text-action hover:underline text-xs font-medium">
              Users &amp; Access
            </Link>
          )}
        </div>
      ...
      <Card padding="none">
        ...
        <div className="divide-y divide-line">
          {matters.map((m) => (
            <Link key={m.id} href={`/admin/matters/${m.id}`} className="flex items-center justify-between px-5 py-3 hover:bg-raised transition-colors">
              <div>
              ...
              <span className="text-xs text-ink-3">{formatDate(m.created_at)}</span>
            </Link>
          ))}
          {matters.length === 0 && (
            <p className="px-5 py-8 text-center text-sm text-ink-3">No matters for this firm yet.</p>
          )}
        </div>
      </Card>
      ...
      <Card padding="none">
        ...
        <div className="divide-y divide-line">
          {transfers.map((t) => (
            <Link key={t.id} href={`/admin/property-transfers/${t.id}`} className="flex items-center justify-between px-5 py-3 hover:bg-raised transition-colors">
              <div>
                <p className="text-sm font-medium text-ink">{t.reference}</p>
                {t.property_description && <p className="text-xs text-ink-3">{t.property_description}</p>}
              </div>
          ...
          {transfers.length === 0 && (
            <p className="px-5 py-8 text-center text-sm text-ink-3">No property transfers for this firm yet.</p>
          )}
        </div>
      </Card>
    </div>
