  ...
  return (
    <div className="max-w-7xl mx-auto">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-[40px] font-semibold leading-[1.06] tracking-[-0.032em] text-ink">Partner Firms</h1>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-line bg-raised">
                <th className="px-5 py-3 text-left text-xs font-semibold text-ink-3 uppercase tracking-wide">Firm</th>
                <th className="px-5 py-3 text-left text-xs font-semibold text-ink-3 uppercase tracking-wide">Code</th>
                <th className="px-5 py-3 text-left text-xs font-semibold text-ink-3 uppercase tracking-wide hidden md:table-cell">Type</th>
                <th className="px-5 py-3 text-left text-xs font-semibold text-ink-3 uppercase tracking-wide hidden lg:table-cell">Email</th>
                <th className="px-5 py-3 text-left text-xs font-semibold text-ink-3 uppercase tracking-wide">Users</th>
            ...
            <tbody className="divide-y divide-line">
              {firms.map((f) => (
                <tr key={f.id} className="hover:bg-raised transition-colors">
                  <td className="px-5 py-3">
                    <Link href={`/admin/firms/${f.id}`} className="font-medium text-ink hover:text-action hover:underline">
                      {f.name}
                    </Link>
                  ...
                  <td className="px-5 py-3">
                    {f.abbreviation ? (
                      <span className="font-mono text-xs text-ink-2">{f.abbreviation}</span>
                    ) : (
                      <span className="text-xs text-amber-600" title="No short code set">Not set</span>
                    )}
                  </td>
                  <td className="px-5 py-3 text-ink-3 hidden md:table-cell">
                    {PARTNER_TYPE_LABELS[f.partner_type] ?? f.partner_type}
                  </td>
                  <td className="px-5 py-3 text-ink-3 hidden lg:table-cell">{f.primary_email ?? "—"}</td>
                  <td className="px-5 py-3 text-ink-2">{userCounts.get(f.id) ?? 0}</td>
                  <td className="px-5 py-3 text-ink-2">{matterCounts.get(f.id) ?? 0}</td>
                  <td className="px-5 py-3 text-right">
                    <Link href={`/admin/firms/${f.id}`} className="text-action hover:underline text-xs font-medium">
